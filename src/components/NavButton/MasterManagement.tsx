...
import { BrifecaseTickIcon } from '~/assets/icons';

const MasterManagementButton = ({ to, active }: INavButtonProps) => {
  const { classes, cx } = useStyles();

  return (
    <Button component={Link} to={to} className={cx(classes.container, active ? classes.active : null)}>
      <Box className={cx(classes.icon)} sx={{ boxShadow: active ? '0px 6px 9px 0px rgba(110, 86, 207, 0.15)' : '' }}>
        <BrifecaseTickIcon />
      </Box>
...
export default MasterManagementButton;
